...
// 全自动打款引擎配置
const TREASURY_MNEMONIC = process.env.TREASURY_MNEMONIC || '';          // 金库助记词，逗号分隔 24 个单词
...
async function payoutJetton(receiveAddress, usdtAmount) {
  ...
  const mnemonic = TREASURY_MNEMONIC.split(',').map(s => s.trim()).filter(Boolean);
...
export async function handleAuth(body) {
  ...
  try {
    ...
    if (action === 'save' && data && typeof data === 'object') {
      ...
      // Adsgram 服务端回调 record_ad_callback 驱动，save 里不再累加，避免重复记账
