...
const WITHDRAW_GAS_FEE = process.env.WITHDRAW_GAS_FEE || '0.13';       // 玩家自付 Gas 下限（GRAM，前端随机 0.13~0.15）
...
// 计算场上猫咪的每秒产出总和（P_n = 1 × 1.8^(n-1)，与前端 totalEarnPerSec 一致；40级满级不产币）
...
//   1. 状态：success === true
//   2. 金额：in_msg.value >= WITHDRAW_GAS_FEE_NANO（默认 0.13 GRAM）
//   3. 收款方：in_msg.dest.address 统一转 Raw 后 == TREASURY_ADDRESS
//   4. 付 gas 地址：in_msg.source 统一转 Raw 后 == 提现接收地址（防代付/地址不一致）
//   5. 身份：in_msg.decoded_body.text 必须包含 tg_id
let _tonapiKeyWarned = false;

async function verifyTonTransfer(hash, tgId, receiveAddress) {
  const gasFeeNano = process.env.WITHDRAW_GAS_FEE_NANO || '130000000';   // 最低门槛 0.13 GRAM（前端随机 0.13~0.15）
  ...
  try {
    // 用消息 hash 查交易：前端 bocToHash 算的是外部消息 hash，必须走 messages/{msg_id}/transaction
    const resp = await fetch('https://tonapi.io/v2/blockchain/messages/' + encodeURIComponent(String(hash)) + '/transaction', { headers });
    ...
    // 身份核对：付 gas 的钱包地址必须 == 提现接收地址（防止代付 / 绑定地址与打款地址不一致）
    let srcAddr = '';
    if (inMsg.source && typeof inMsg.source === 'object') srcAddr = inMsg.source.address || '';
    else if (typeof inMsg.source === 'string') srcAddr = inMsg.source;

    if (!toRaw(srcAddr) || toRaw(srcAddr) !== toRaw(receiveAddress)) return false;
...
export async function handleAuth(body) {
  ...
  try {
    ...
    if (action === 'request_withdraw') {
      ...
      // ① 验账（TonAPI 强校验：状态/金额/收款方/付gas地址==接收地址/memo）
      const verified = await verifyTonTransfer(bocOrHash, tgId, receiveAddress);
      if (!verified) {
